...
class DetailSatuanView extends GetView<SatuanController> {
  const DetailSatuanView({super.key});
  ...
  Widget build(BuildContext context) {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      controller.fetchSatuanById(id);
    });

    return Scaffold(
      appBar: Styles.bar("DETAIL SATUAN"),
      body: Obx(() {
        if (controller.isLoading.value) {
          return const Center(child: CircularProgressIndicator());
        }

        final satuan = controller.satuanDetail.value;
        if (satuan == null) {
                  ...
                  children: [
                    ...
                    Column(
                      children: [
                        IconButton(
                          onPressed: () {
                            Get.toNamed(Routes.KELOLA_EDIT_SATUAN, arguments: satuan);
                          },
...
Widget buildDetailTable(Map<String, dynamic> user) {
  return Table(
    ...
    children: [
      buildDetailRow("Nama", user['namasatuan']),
