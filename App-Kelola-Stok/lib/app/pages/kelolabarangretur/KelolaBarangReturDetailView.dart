...
class KelolaBarangReturDetailView extends GetView<ReturController> {
  const  KelolaBarangReturDetailView({super.key});


  @override
  Widget build(BuildContext context) {
    WidgetsBinding.instance.addPostFrameCallback((_) {
      controller.fetchDetailReturById(Get.arguments['id_retur']);
    });

    final header = Get.arguments;

    return Scaffold(
      appBar: Styles.bar("Detail Retur"),
      body: Obx(() {
        if (controller.isLoading.value) {
          return const Center(child: CircularProgressIndicator());
        }

        final detailList = controller.detailReturList;
        if (detailList.isEmpty) {
          return const Center(child: Text("Detail Retur tidak ditemukan"));
        }
              ...
              Card(
                ...
                child: Padding(
                  ...
                  child: Table(
                    ...
                    children: [
                      buildDetailRow("Nama Pembuat", header['name']),
                      buildDetailRow("Nama Agen Supplier", header['namasupplier']),
                      buildDetailRow("Nama Tipe", header['namatipe']),
                      buildDetailRow("Total Refund", Styles.formatMoneyRp(header['totalrefund'])),
                      buildDetailRow("Tanggal Buat", header['createat']),
                    ],
                  ),
                ),
              ),
